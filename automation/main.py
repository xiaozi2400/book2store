...
from automation.progress import phase
...
def scan():
    ...
    if books:
        ...
        for book in books:
            table.add_row(book.get("title", "Unknown"), book.get("author", "Unknown"), book.get("language", "en"))
...
@app.command()
def list(
    status_filter: Optional[str] = typer.Option(None, help="按状态筛选"), limit: int = typer.Option(10, help="显示数量")
):
    """列出书籍"""
    db = DatabaseManager()
...
@app.command()
def process(
    book_id: str = typer.Argument(..., help="书籍ID"), skip_publish: bool = typer.Option(False, help="跳过发布步骤")
):
    """处理指定书籍"""
    import time
...
@app.command()
def import_links_cmd(excel_path: str = typer.Argument(..., help="Excel文件路径")):
    """从Excel导入分享链接"""
...
@app.command()
def publish(
    book_id: str = typer.Argument(..., help="书籍ID"), auto: bool = typer.Option(False, help="自动发布（跳过手动确认）")
):
    """发布到闲鱼"""
...
@app.command()
def generate_list(book_id: str = typer.Argument(..., help="书籍ID")):
    """生成闲鱼发布清单（备用方案）"""
...
def list_failed():
    ...
    try:
        failed_books = (
            ...
            .options(joinedload(Book.outputs))
            .filter(BookOutput.publish_status == "failed")
...
def test(
    ...
    console.print("[bold yellow]测试模式[/bold yellow]")
    ...
    for i, epub_path in enumerate(epub_files, 1):
        ...
        try:
            ...
            if not skip_translate:
                if skip_cache:
                    console.print("  [dim]→ 翻译并生成PDF（跳过缓存）...[/dim]")
                else:
                    console.print("  [dim]→ 翻译并生成PDF...[/dim]")
                if not translate_book(book_id, str(epub_path), skip_cache=skip_cache):
                    raise TranslationError("翻译失败", book_id)
            else:
                console.print("  [dim]→ 跳过翻译步骤[/dim]")

            if not skip_summarize:
                console.print("  [dim]→ 生成精简版...[/dim]")
                generate_summary(book_id, str(epub_path))
            else:
                console.print("  [dim]→ 跳过精简版步骤[/dim]")

            if not skip_images:
                console.print("  [dim]→ 生成主图...[/dim]")
                from automation.image import generate_main_image

                if not generate_main_image(book_id):
                    console.print("  [yellow]⚠ 主图生成失败（检查日志）[/yellow]")
            else:
                console.print("  [dim]→ 跳过主图步骤[/dim]")

            if not skip_copywriting:
                console.print("  [dim]→ 生成文案...[/dim]")
                book_obj = db.get_book_by_id(book_id)
                generate_copywriting(
                    book_id,
                    {"title": book_obj.title, "author": book_obj.author, "summary": book_obj.summary_text or ""},
                )
            else:
                console.print("  [dim]→ 跳过文案生成步骤[/dim]")

            console.print("  [dim]→ 跳过闲鱼发布（测试模式）[/dim]")
...
def regenerate_images(
    ...
    for book in target_books:
        ...
        if generate_main_image(book.id):
            console.print("  [green]✓[/green] 主图生成完成")
        else:
            console.print("  [red]✗[/red] 主图生成失败")
