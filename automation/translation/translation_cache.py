...
class SQLiteTranslationCache(TranslationCache):
    ...
    def get(self, text):
        ...
        try:
            record = (
                session.query(TranslationCacheModel).filter(TranslationCacheModel.source_hash == source_hash).first()
            )
        ...
        try:
            ...
            for cache_hash, cache_item in cache_data.items():
                ...
                if source_text and translated_text:
                    # 检查是否已存在
                    existing = (
                        session.query(TranslationCacheModel)
                    ...
                    if not existing:
                        new_record = TranslationCacheModel(
                            source_hash=cache_hash, source_text=source_text, translated_text=translated_text
                        )
                        session.add(new_record)
                        count += 1
                        if count % 100 == 0:
