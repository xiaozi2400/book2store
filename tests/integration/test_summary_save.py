...
class TestSummarySave:
    ...
    def test_summary_saved_when_generate_succeeds(self):
        ...
        # Mock 适合度检查
        summarizer._check_suitability = MagicMock(
            return_value=Mock(passed=True, label="适合", score=85, recommendation="推荐", details={})
        )
        ...
        # Mock 封面提取和章节提取
        with (
            patch("automation.summarizer.content_summarizer.extract_cover_from_epub", return_value=None),
            patch(
                "automation.summarizer.content_summarizer.extract_chapters",
                return_value=[
    ...
    def test_summary_not_saved_when_generate_returns_none(self):
        ...
        session = get_session()
        book = Book(id="test-book-002", filename="test.epub", title="测试书籍", author="测试作者", status="pending")
        session.add(book)
        session.commit()
        session.close()
