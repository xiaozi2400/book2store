...
def _make_translator() -> Translator:
    ...
    t.provider = "test"
    t.stats = {"api_calls": 0, "total_tokens": 0, "translated_paragraphs": 0, "retried": 0, "failed": 0, "by_tier": {}}
    t.opt_config = {
        "batch": {
            "tier1_batch_size": 10,
            "tier2_batch_size": 5,
...
def test_whitespace_only_translation_keeps_original():
    """空格翻译应保留原文"""
    t = _make_translator()
    result = {"id": 2, "original": "Chapter 1", "translated": "   ", "html": "<p>Chapter 1</p>", "tier": "tier2_normal"}
    checked = t._quality_check(result, "tier2_normal")
...
def test_too_short_absolute_keeps_translated():
    ...
    t = _make_translator()
    result = {"id": 3, "original": "Hi", "translated": "x", "html": "<p>Hi</p>", "tier": "tier2_normal"}
    checked = t._quality_check(result, "tier2_normal")
...
def test_short_translation_keeps_translated_not_original():
    ...
    checked = t._quality_check(result, "tier2_normal")
    assert (
        checked["translated"] == "1 原子习惯的惊人力量"
    ), f"翻译过短但有效的应保留翻译，实际得到: {checked['translated']}"
    assert checked["translated"] != result["original"], "不应回退到原文"
...
def test_ratio_too_low_long_text_keeps_translated():
    ...
    checked = t._quality_check(result, "tier2_normal")
    assert checked["translated"] == "短译文", f"翻译过短应保留翻译，实际得到: {checked['translated']}"
