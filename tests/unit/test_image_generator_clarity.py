...
def _build_generator_with_config(cfg: dict) -> ImageGenerator:
    ...
    with (
        patch("automation.image.image_generator.config") as mock_config,
...
def test_screenshot_passes_device_scale_factor(mock_sync_playwright):
    ...
    assert kwargs.get("viewport") == {
        "width": 800,
        "height": 800,
    }, f"viewport 应是 {{'width': 800, 'height': 800}},实际 {kwargs.get('viewport')}"
    assert (
        kwargs.get("device_scale_factor") == 2
...
def test_screenshot_passes_jpeg_quality(mock_sync_playwright):
    ...
    assert ckwargs.get("type") == "jpeg", f"screenshot type 应是 'jpeg',实际 {ckwargs.get('type')}"
    assert ckwargs.get("quality") == 92, f"screenshot quality 应是 92,实际 {ckwargs.get('quality')}"
