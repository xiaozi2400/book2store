...
def translate_epub(
    epub_path, output_dir=".", book_id=None, skip_cache=False, test_mode=False, cache=None, progress_callback=None
):
    """
    翻译 EPUB 电子书（库函数版本）
    ...
    if paragraphs_to_translate:
        ...
        if translated_results:
            fresh_results = [
                r for r in translated_results if not r.get("is_duplicate") and not r.get("_needs_original_translation")
            ]
            if fresh_results and all(not r.get("translated") or not r["translated"].strip() for r in fresh_results):
    ...
    cache_data = {
        str(i): {"source": r["original"], "translated": r["translated"]} for i, r in enumerate(all_translated)
    }
...
def convert_english_pdf_only(epub_path, output_dir="."):
    """
