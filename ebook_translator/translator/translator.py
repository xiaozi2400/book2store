    ...
    def chat(self, prompt: str, max_retries: int = 3, max_tokens: int = None) -> str:
        ...
        while retry_count < max_retries:
            try:
                ...
                retry_count += 1
                logger.warning(
                    f"AI chat调用失败 (重试 {retry_count}/{max_retries}): {type(e).__name__}: {str(e)[:200]}"
                )
                if retry_count < max_retries:
                    wait_time = min(2**retry_count, 30)
                    time.sleep(wait_time)
    ...
    def chat_raw(self, prompt: str, max_retries: int = 3, max_tokens: int = None) -> dict:
        ...
        while retry_count < max_retries:
            try:
                ...
                retry_count += 1
                logger.warning(
                    f"AI chat_raw 调用失败 (重试 {retry_count}/{max_retries}): {type(e).__name__}: {str(e)[:200]}"
                )
                if retry_count < max_retries:
                    wait_time = min(2**retry_count, 30)
                    time.sleep(wait_time)
...
3. 保持段落顺序"""
    ...
    def translate_smart(self, paragraphs):
        ...
        tier_groups = {"tier1_short_repeatable": [], "tier2_normal": [], "tier3_long_complex": []}
        ...
        total_paragraphs = (
            len(tier_groups["tier1_short_repeatable"])
        ...
        with Progress(
            ...
            if tier_groups["tier2_normal"]:
                ...
                tier2_results = self._translate_tier(
                    tier_groups["tier2_normal"], "tier2_normal", batch_config["tier2_batch_size"], progress, task
                )
            ...
            if tier_groups["tier3_long_complex"]:
                ...
                tier3_results = self._translate_tier(
                    tier_groups["tier3_long_complex"], "tier3_long_complex", 1, progress, task
                )
    ...
    def _translate_tier(self, paragraphs, tier, batch_size, progress=None, task=None):
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_batch = {executor.submit(self.translate_batch, batch, tier): batch for batch in batches}
        ...
        if issues:
            ...
            if len(original) > 50:
                ...
                if retry_result and len(retry_result) > len(translated) * 0.5:
                    ...
                    logger.info("  重译失败或质量无改善，保留原结果")
                    self.stats["failed"] += 1
            else:
                logger.info("  短文本质量异常，跳过重试")
                self.stats["failed"] += 1
